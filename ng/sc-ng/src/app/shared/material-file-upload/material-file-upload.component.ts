...
export class MaterialFileUploadComponent implements OnInit {
      ...
      @Input('module') moduelName = 'user';
      /** Link text */
      @Input('text') text = 'Upload';
      /** Name used in form which will be sent in HTTP request. */
      @Input('param') param = 'file';
      ...
      @Output('complete') complete = new EventEmitter<string>();
      @Input('uploadedFileUrl') uploadedFileUrl = '';
      ...
      private uploadFile(file: FileUploadModel) {
            ...
            let uploadURL = this.appservice.userBaseURL + this.target;
            if(this.moduelName == 'course'){
                  uploadURL = this.appservice.courseBaseURL + this.target;
            }
            ...
            file.sub = this._http.request(req).pipe(
                  ...
                  (event: any) => {
                        if (typeof (event) === 'object') {
                              this.removeFileFromArray(file);
                              this.uploadedFileUrl = event.body.data.cloudFrontPath;
